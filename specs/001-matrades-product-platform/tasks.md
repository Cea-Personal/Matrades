...
## Phase 14: Extras and Notification Delivery

**Purpose**: Make generated strategy code and timestamped research evidence discoverable in the UI,
and provide encrypted, testable Telegram and Pushover delivery channels.

- [X] T268 [HIGH] Render deterministic, read-only strategy evaluator source from every canonical specification and persist its language/hash alongside strategy versions and backtest evidence in `modules/strategies/compiler.py`, `apps/api/app/routes/strategies.py`, and `apps/worker/app/tasks/strategies.py`
- [X] T269 [HIGH] Add owner-scoped Extras archive aggregation endpoints for market research, strategy research/backtests, trade recommendations with rationale/evidence, and generated code, including safe manifest reads in `apps/api/app/routes/extras.py` and `modules/research/artifacts.py`
- [X] T270 [HIGH] Add the Extras sidebar tab and folder navigation/detail views with generated-code rendering, cycle summaries, immutable archive references, recommendation reasons, and loading/error/empty states in `apps/web/src/app/layout.tsx`, `apps/web/src/app/extras/page.tsx`, and `apps/web/src/features/extras/Extras.tsx`
- [X] T271 [HIGH] Implement encrypted Telegram and Pushover channel resources, preferences, provider test receipts, health/error state, MFA-protected update/removal, and masked UI configuration in `modules/notifications/providers.py`, `apps/api/app/routes/operations.py`, and `apps/web/src/features/notifications/NotificationCenter.tsx`
- [X] T272 [HIGH] Replace the notification delivery stub with preference-aware worker delivery, urgent-only filtering, redacted receipts/failures, and durable delivery state transitions in `apps/worker/app/tasks/operations.py`
- [X] T273 [HIGH] Add contract/unit coverage for Extras routes, generated source validity, Telegram/Pushover provider payloads, and notification channel operations in `tests/contract/`, `tests/unit/`, and `tests/e2e/`

## Phase 15: Runtime, Strategy, and Knowledge Expansion

**Purpose**: Make runtime selection observable, enable LiteLLM as a persisted explicit alternative,
and make documents, transcripts, SerpApi discovery, and generated strategies first-class indexed
knowledge without duplicate ingestion.

- [X] T274 [HIGH] Persist encrypted agent runtime settings, expose Codex heartbeat and per-agent actual-runtime execution status, and load saved settings in the isolated agent worker while preserving Codex-first defaults in `apps/api/app/routes/agents.py`, `modules/agents/runtime.py`, and `apps/agent_worker/app/main.py`
- [X] T275 [HIGH] Add UI controls for enabling/testing LiteLLM, configuring its URL/key, restarting the worker, and showing configured versus actual runtime evidence in `apps/web/src/features/agents/ModelProfiles.tsx` and `apps/web/src/features/agents/AgentConfiguration.tsx`
- [X] T276 [HIGH] Persist approved generated strategies as owner-scoped, timestamped knowledge sources with generated code, strategy family/origin metadata, deterministic embeddings, vector-index state, and a version link in `apps/api/app/routes/strategies.py` and `modules/knowledge/ingestion.py`
- [X] T277 [HIGH] Add secure document/transcript upload extraction for PDF, DOCX, TXT, Markdown, VTT, and SRT with blob retention, content-hash idempotency, chunk embeddings, and UI upload controls in `apps/api/app/routes/knowledge.py`, `modules/security/platform.py`, and `apps/web/src/features/knowledge/KnowledgeSources.tsx`
- [X] T278 [HIGH] Add an encrypted SerpApi connection provider, YouTube discovery/transcript ingestion through `youtube-transcript-api`, source/video deduplication, failure reporting, and knowledge UI controls in `modules/connections/`, `modules/knowledge/youtube.py`, `apps/api/app/routes/knowledge.py`, and `apps/web/src/features/configuration/Connections.tsx`
- [X] T279 [HIGH] Upgrade knowledge retrieval and health reporting to expose deterministic vector/lexical hybrid scores, embedding provenance, indexed segment counts, and transcript/document capabilities while preserving context-only authority and owner filters
- [X] T280 [HIGH] Add unit, contract, and browser-facing coverage for runtime evidence/settings, SerpApi provider registration, YouTube ID parsing/deduplication, uploads, hybrid retrieval, and generated-strategy indexing
- [X] T281 [HIGH] Expose the bundled read-only `MatradesMT5BridgeEA.mq5` as an authenticated Extras folder artifact with source preview, SHA-256 metadata, and download endpoint in `apps/api/app/routes/extras.py`, `apps/web/src/features/extras/Extras.tsx`, and `apps/web/src/app/layout.tsx`
