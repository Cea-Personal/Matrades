...
**Goal**: Authenticate an owner, connect one primary account, configure external and stricter
internal limits, and show deterministic shared-equity risk and capacity.

**Independent Test**: From a new installation, an owner signs in with MFA, configures one account
and both rule sets through the UI, and sees risk state/capacity; anonymous and unauthorized access
fails, and stricter internal limits block before prop limits.
...
**Goal**: Detect manually created broker positions, classify and include them in shared risk,
freeze recommended trade theses, and monitor evidence without executing orders.
...
- **US1 — Safe Trading Account**: Starts after Foundational and establishes production account/risk
  truth used by later live workflows.
- **US2 — Active Markets**: Starts after Foundational; uses account/prop fixtures independently,
  then integrates with US1 for production eligibility.
...
- **US6 — Manual Monitoring**: Depends on US1 account/risk and US5 recommendation structures; its
  discretionary-position path remains independently testable with broker fixtures.
...
- **US9 — UI Operations and Audit**: Builds on Foundation audit/jobs/integration ports and integrates
  all completed story health/events; each operations flow remains fixture-testable.
...
US4 + US6 -> US7
US2 + US3 + US4 -> US8
Foundation -> US9, then integrate US1..US8 events and health
...
- Cross-cutting release tests T219–T226 can be developed concurrently against the integrated build.
...
```text
Parallel tests: T037, T038, T039, T040, T041, T042
Parallel models: T043, T044, T045
Parallel interfaces after domain services: T054, T055, T056, T057
```

### User Story 2

```text
Parallel tests: T060, T061, T062, T063, T064, T065
Parallel models: T066, T067
Parallel interfaces after market services: T077, T078, T079, T080
```

### User Story 3

```text
Parallel tests: T082, T083, T084, T085, T086, T087, T088
Parallel models: T089, T090, T091
Parallel validators after runtime: T101, T102, T103
Parallel interfaces after validation service: T106, T107, T108, T109, T110, T111
```

### User Story 4

```text
Parallel tests: T113, T114, T115, T116, T117
Parallel models: T118, T119
Parallel interfaces after services: T125, T126, T127, T128
```

### User Story 5

```text
Parallel tests: T130, T131, T132, T133, T134, T135
Parallel models: T136, T137
Parallel interfaces after recommendation service: T145, T146, T147
```

### User Story 6

```text
Parallel tests: T149, T150, T151, T152, T153, T154
Parallel models: T155, T156
Parallel interfaces after monitor service: T163, T164, T165
```

### User Story 7

```text
Parallel tests: T167, T168, T169, T170
Parallel API, worker, and primary UI after services: T177, T178, T179
```

### User Story 8

```text
Parallel tests: T181, T182, T183, T184
Parallel API, worker, and replacement UI after services: T190, T191, T192
```

### User Story 9

```text
Parallel tests: T194, T195, T196, T197, T198, T199, T200
Parallel models: T201, T202, T203
Parallel routes/adapters/UI after services: T209, T210, T211, T212, T213, T214, T215, T216, T217
```
...
3. Complete US1 through T059.
4. Stop and run every US1 test and the account/risk portion of `quickstart.md`.
5. Demonstrate authenticated UI-only account/risk control; do not describe it as trading-ready.
