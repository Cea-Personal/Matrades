...
_APPROVED: dict[str, ProviderDefinition] = {
    ...
    "CME_GROUP": ProviderDefinition(
        ...
        lifecycle="OPTIONAL",
        licensing_notice="Optional future CME market-data entitlement and display/retention rights are required.",
    ...
    "CBOE_FX_SPOT": ProviderDefinition(
        ...
        lifecycle="RETIRED",
        licensing_notice="Retired historical metadata only; it cannot be newly connected or selected.",
        credential_required=True,
        entitlement_required=True,
        verification_only=True,
    ...
    "TWELVE_DATA": ProviderDefinition(
        ...
        adapter_revision="twelve-data-rest-v1",
        asset_categories=frozenset({"FOREX", "CRYPTO"}),
        capability_semantics=(
            ("QUOTES", "AGGREGATED_PROXY"),
            ("CANDLES", "AGGREGATED_PROXY"),
            ("TRADED_VOLUME", "AGGREGATED_PROXY"),
        ),
        licensing_notice=(
            "Aggregate forex/crypto price and candle fallback only. It does not provide "
            "venue order-book or executed-volume authority."
        ),
        credential_required=True,
        fixed_base_url="https://api.twelvedata.com",
    ),
    "FEDERAL_RESERVE": ProviderDefinition(
        provider="FEDERAL_RESERVE",
        category="ECONOMIC_CALENDAR",
        capabilities=frozenset({"ECONOMIC_CALENDAR_READ"}),
        official_source_required=True,
        configuration_fields=frozenset(), credential_fields=frozenset(),
        lifecycle="OWNER_CITED_ONLY", verification_only=True,
        licensing_notice="Official cited FOMC schedule; no production HTML scraping.",
        fixed_base_url="https://www.federalreserve.gov",
    ),
    "BLS": ProviderDefinition(
        provider="BLS", category="ECONOMIC_CALENDAR",
        capabilities=frozenset({"ECONOMIC_CALENDAR_READ"}), official_source_required=True,
        configuration_fields=frozenset(), credential_fields=frozenset(),
        capability_semantics=(("SCHEDULE", "AUTHORITATIVE"), ("RELEASED_VALUE", "AUTHORITATIVE")),
        licensing_notice="Official BLS ICS schedule and public released values; no HTML scraping.",
        fixed_base_url="https://www.bls.gov",
    ),
    "BEA": ProviderDefinition(
        provider="BEA", category="ECONOMIC_CALENDAR",
        capabilities=frozenset({"ECONOMIC_CALENDAR_READ"}), official_source_required=True,
        configuration_fields=frozenset(), credential_fields=frozenset(),
        capability_semantics=(("SCHEDULE", "AUTHORITATIVE"), ("RELEASED_VALUE", "AUTHORITATIVE")),
        licensing_notice="Official BEA machine-readable calendar and released values; no HTML scraping.",
        fixed_base_url="https://www.bea.gov",
    ),
    "EIA": ProviderDefinition(
        provider="EIA", category="ECONOMIC_CALENDAR",
        capabilities=frozenset({"ECONOMIC_CALENDAR_READ"}), official_source_required=True,
        configuration_fields=frozenset(), credential_fields=frozenset({"api_key"}),
        lifecycle="OWNER_CITED_SCHEDULE", verification_only=True,
        capability_semantics=(("RELEASED_VALUE", "AUTHORITATIVE"),),
        licensing_notice="Official EIA values; schedules are owner-cited because no documented machine schedule feed is used.",
        credential_required=True, fixed_base_url="https://api.eia.gov",
    ),
    "FOREX_FACTORY_SCRAPER": ProviderDefinition(
        ...
        configuration_fields=frozenset(),
        credential_fields=frozenset(),
        catalogue_revision="2026-08-20.experimental.v1",
        adapter_revision="forex-factory-scraper-api-v1",
        lifecycle="EXPERIMENTAL_NON_PRODUCTION",
        verification_only=True,
        capability_semantics=(("SCHEDULE", "SCRAPED_EXPERIMENTAL"),),
        licensing_notice=(
            "Development-only scraper experiment. Its results are never official calendar "
            "coverage, risk-gate evidence, or live recommendation input."
        ),
        fixed_base_url="http://forex-factory-scraper:5000",
    ),
...
_FRESHNESS_SECONDS: dict[str, dict[str, int]] = {
    "MT5_TERMINAL_BRIDGE": {"QUOTES": 60, "INSTRUMENTS": 60, "CANDLES": 3600, "LIQUIDITY": 60},
    "CME_GROUP": {"TRADED_VOLUME": 86400, "OPEN_INTEREST": 86400, "ORDER_BOOK": 60},
    "TWELVE_DATA": {"QUOTES": 300, "CANDLES": 3600, "TRADED_VOLUME": 300},
