...
- Name: apps/web/tests/e2e/safe_account.spec.ts >> first-owner signup presents a complete, usable form
- Location: apps/web/tests/e2e/safe_account.spec.ts:25:5
...
> 30  |   await page.goto("/setup");
  ...
  116 |   await page.getByRole("button", { name: "Record account identity" }).click();
  117 |   await expect(page.getByRole("heading", { name: "Record external loss rules" })).toBeVisible();
  118 | });
  119 | 
```