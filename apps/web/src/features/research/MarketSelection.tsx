  ...
  return <section className="section-stack">
    ...
    <article className="card form-stack">
      ...
      {enabledLanes.length ? <div className="grid two">{enabledLanes.map(lane => {
        ...
        return <article className="card" key={`${lane.asset_class}:${lane.instrument_type}`}><strong>{lane.asset_class} · {lane.instrument_type}</strong><p className={result?.status === "READY" ? "good" : "muted"}>{result?.status ?? "NOT_RUN"}</p>{result?.candidate ? <><p>{result.candidate.listing.symbol} · {result.candidate.listing.venue}</p><small>Score {result.candidate.score.toFixed(2)}</small></> : <small>{result?.reason_code ?? "No completed lane result yet"}</small>}</article>;
    ...
    {typedRuns.isPending ? <p>Loading research…</p> : latest ? <>
    ...
    <article className="card"><h2>Cycle outcome</h2><p className="muted">The latest completed cycle, candidates, safe terminal statuses, and evidence are retained in the research archive.</p></article>
    <article className="card"><h2>Market research archive</h2><p className="muted">Every completed cycle is saved with an explicit immutable completion timestamp.</p>{artifacts.data?.length ? <ul className="record-list">{artifacts.data.map(item => <li key={item.run_id}><strong>{item.state}</strong><span>{item.relative_path}</span><small>Completed timestamp: {new Date(item.completed_at).toLocaleString()} · SHA-256 {item.checksum.slice(0, 12)}…</small></li>)}</ul> : <p className="empty">No archived market-research cycles yet.</p>}</article>
  </section>;
}
