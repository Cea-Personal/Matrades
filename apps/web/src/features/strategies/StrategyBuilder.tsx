...
export function StrategyBuilder() {
  ...
  const create = useMutation({
    mutationFn: () => api<Resource>("/strategies/drafts", { method: "POST", body: JSON.stringify({ origin, description: description || null }) }),
    onSuccess: async () => { setDescription(""); setMessage("Strategy research queued. The assigned AI agent will propose a family and complete rules."); await refresh(); },
    onError: (error: Error) => setMessage(error.message),
  });
  const decide = useMutation({
    mutationFn: ({ id, action }: { id: string; action: string }) => api<Resource>(`/strategies/drafts/${id}/proposal-decision`, { method: "POST", body: JSON.stringify({ action }) }),
    onSuccess: async item => { setMessage(item.state === "DRAFT" ? "Strategy proposal approved and made canonical." : "Strategy proposal rejected."); await refresh(); },
    onError: (error: Error) => setMessage(error.message),
  });
  ...
  return <section className="section-stack">
    <header><p className="eyebrow">AI strategy lifecycle</p><h1>Strategy research & validation</h1><p className="muted">AI proposes; you approve; deterministic services compile, backtest, validate, and gate promotion.</p></header>
    <article className="card form-stack"><h2>Research basis</h2><p className="muted">Strategy research is pinned to the latest approved market selection and provider history; you do not choose another pair here.</p>{researchContext.isPending ? <p>Checking approved research evidence…</p> : researchContext.data?.ready ? <dl><dt>Approved instrument</dt><dd>{researchContext.data.instrument} · {researchContext.data.category}</dd><dt>Market evidence observed</dt><dd>{researchContext.data.market_observed_at ? new Date(researchContext.data.market_observed_at).toLocaleString() : "—"}</dd><dt>Historical provider</dt><dd>{researchContext.data.historical_provider}</dd></dl> : <p className="notice">Not ready: {researchContext.data?.reason ?? "research context unavailable"}. Run the autonomous market cycle, approve HIL-1, and configure its historical provider.</p>}</article>
    <form className="card form-stack" onSubmit={submitGeneration}><h2>Generate strategy</h2><div className="form-grid"><label>Creation mode<select value={origin} onChange={event => setOrigin(event.target.value as Origin)}><option value="AI_GENERATED">AI Generated</option><option value="AI_ASSISTED">AI Assisted</option></select></label>{origin === "AI_ASSISTED" ? <label>Your strategy idea<textarea required rows={5} placeholder="Describe the behavior, timing, and risk preferences; the approved instrument remains fixed…" value={description} onChange={event => setDescription(event.target.value)} /></label> : <label>Optional strategy focus<textarea rows={5} placeholder="For example: conservative intraday momentum with tight invalidation" value={description} onChange={event => setDescription(event.target.value)} /></label>}</div><p className="muted">AI Generated uses the Strategy Researcher to develop three evidence-cited families or hypotheses. AI Assisted uses your idea and the Strategy Assistant. A deterministic unseen holdout screen selects the proposal shown for human approval; this preliminary screen is not formal validation.</p><button className="btn primary" disabled={create.isPending || !researchContext.data?.ready}>{create.isPending ? "Queuing…" : "Generate strategy"}</button></form>
    {message && <p className="notice">{message}</p>}

    <section className="section-stack"><h2>Strategy proposals</h2><p className="muted">Review the family, rules, evidence, and breakdown. You may Accept, Edit after approval, or Reject; Similarity screening runs before a permanent version is created.</p>{drafts.isPending ? <p>Loading strategy research…</p> : drafts.data?.length ? <div className="grid">{drafts.data.map(item => {
      ...
      return <article className="card form-stack" key={item.id}><div><p className="muted">{String(item.origin).replaceAll("_", " ")}</p><h3>{String(proposed?.name ?? "Research in progress")}</h3><span className={`status ${item.state.toLowerCase()}`}>{item.state}</span></div>{proposed ? <dl><dt>Strategy family</dt><dd>{String(proposed.family)}</dd><dt>Horizon</dt><dd>{String(proposed.horizon)}</dd><dt>Instruments</dt><dd>{(proposed.instruments as string[]).join(", ")}</dd><dt>Risk per trade</dt><dd>{String(proposed.risk_per_trade)}%</dd></dl> : <p className="muted">The bounded agent is preparing three grounded hypotheses.</p>}{pack ? <><h4>Research basis</h4><dl><dt>Approved market fingerprint</dt><dd>{String(pack.instrument)} · {String(fingerprint?.regime)} · {String(fingerprint?.source)}</dd><dt>Discovery history</dt><dd>{String(discovery?.timeframe)} · checksum {String(discovery?.checksum).slice(0, 12)}…</dd><dt>Selected hypothesis</dt><dd>{screen?.selected_hypothesis_id ?? "Pending unseen holdout screen"}</dd></dl>{evidence.length ? <p className="muted">Evidence references: {evidence.join(" · ")}</p> : null}{screen?.results?.length ? <><h4>Preliminary alternatives</h4><ul>{screen.results.map(result => <li key={result.hypothesis_id}>{result.hypothesis_id}: score {result.score} · {result.trade_count} holdout trades · {result.eligible ? "eligible" : result.reasons.join(", ")}</li>)}</ul></> : null}</> : null}{breakdown.length ? <><h4>Step-by-step breakdown</h4><ol>{breakdown.map(step => <li key={step}>{step}</li>)}</ol></> : null}<div className="actions">{item.state === "AWAITING_STRATEGY_APPROVAL" ? <><button className="btn primary" disabled={decide.isPending} onClick={() => decide.mutate({ id: item.id, action: "APPROVE" })}>Approve strategy</button><button className="btn" disabled={decide.isPending} onClick={() => decide.mutate({ id: item.id, action: "REJECT" })}>Reject</button></> : null}{item.state === "DRAFT" ? <button className="btn primary" disabled={submit.isPending} onClick={() => submit.mutate(item.id)}>Submit canonical version</button> : null}</div></article>;
