...
export function NotificationCenter() {
  ...
  return <section className="section-stack">
    ...
    <article className="card"><h3>In-product inbox</h3>{notifications.isPending ? <p>Loading…</p> : notifications.data?.length ? <ul className="record-list">{notifications.data.map(item => <li key={item.id}><strong>{String(item.title ?? item.event_type ?? "Notification")}</strong><span>{String(item.message ?? item.body ?? "")}</span>{!item.read && <button className="btn compact" onClick={() => markRead.mutate(item.id)}>Mark read</button>}</li>)}</ul> : <p className="empty">No notifications.</p>}</article>
