...
type AccountSchedule = { account_id: string; enabled: boolean; run_at: string; timezone: string; weekdays: number[]; next_run_at: string | null };
...
export function Connections() {
  ...
  const replaceCredential = useMutation({
    mutationFn: (input: CredentialReplacement) => api<Resource>(`/configuration/credentials/${input.id}`, { method: "PATCH", body: JSON.stringify({ name: input.name, provider: input.provider, purpose: input.purpose, secret: input.secret }) }),
    onSuccess: async item => { setReplacement(null); setMessage(`${String(item.name)} replaced. Test the linked connection for a fresh provider check.`); await invalidate(); },
    onError: (error: Error) => setMessage(error.message),
  });
  ...
  const saveScraperSchedule = useMutation({
    ...
    onSuccess: async result => { setScraperScheduleDraft(result); setMessage("Forex Factory scraper schedule saved for this trading account."); await queryClient.invalidateQueries({ queryKey: ["configuration", "account", selectedScraperAccountId, "forex-factory-schedule"] }); },
    onError: (error: Error) => setMessage(error.message),
  });
  ...
  return <section className="section-stack">
    ...
    <article className="card form-stack"><h2>Forex Factory scraper schedule</h2><p className="muted">The scheduler checks every minute and runs the scraper independently for each trading account. A period already archived is skipped without another provider request.</p>{accounts.data?.length ? <><label>Trading account<select value={selectedScraperAccountId} onChange={event => { setScraperAccountId(event.target.value); setScraperScheduleDraft(null); }}><option value="">Select an account</option>{accounts.data.map(account => <option key={account.id} value={account.id}>{String(account.name)}</option>)}</select></label>{editableScraperSchedule ? <><div className="form-grid"><label>Run time<input type="time" value={editableScraperSchedule.run_at} onChange={event => changeScraperSchedule({ run_at: event.target.value })} /></label><label>Timezone<input value={editableScraperSchedule.timezone} placeholder="Africa/Kigali" onChange={event => changeScraperSchedule({ timezone: event.target.value })} /></label></div><label><input type="checkbox" checked={editableScraperSchedule.enabled} onChange={event => changeScraperSchedule({ enabled: event.target.checked })} /> Enable scheduled scraper</label><fieldset><legend>Run on</legend><div className="actions">{["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"].map((day, index) => <label key={day}><input type="checkbox" checked={editableScraperSchedule.weekdays.includes(index)} onChange={event => changeScraperSchedule({ weekdays: event.target.checked ? [...new Set([...editableScraperSchedule.weekdays, index])].sort() : editableScraperSchedule.weekdays.filter(item => item !== index) })} /> {day}</label>)}</div></fieldset><button className="btn" disabled={saveScraperSchedule.isPending || (editableScraperSchedule.enabled && editableScraperSchedule.weekdays.length === 0)} onClick={() => saveScraperSchedule.mutate()}>{saveScraperSchedule.isPending ? "Saving…" : "Save scraper schedule"}</button><p className="muted">Next run: {editableScraperSchedule.next_run_at ? new Date(editableScraperSchedule.next_run_at).toLocaleString() : "Schedule disabled"}</p></> : <p className="empty">Select an account to configure its scraper schedule.</p>}</> : <p className="empty">Configure a trading account first.</p>}</article>
    ...
    <article className="card"><h2>Stored credentials</h2><p className="muted">Secrets remain •••• masked after entry. Testing a connection also updates its linked credential status. Replace a key here to keep existing connections linked while resetting their health cache.</p>{credentials.data?.length ? <ul className="record-list">{credentials.data.map(item => <li key={item.id}><strong>{String(item.name)}</strong><span>{String(item.provider)} · {String(item.masked_suffix)}</span><small>{String(item.status)}{item.last_tested ? ` · tested ${new Date(String(item.last_tested)).toLocaleString()}` : ""}</small><div className="actions"><button className="btn compact" disabled={testCredential.isPending} onClick={() => testCredential.mutate(item.id)}>Test credential</button><button className="btn compact" onClick={() => setReplacement({ id: item.id, name: String(item.name), provider: item.provider as Provider, purpose: String(item.purpose ?? "market_data"), secret: "" })}>Replace</button></div></li>)}</ul> : <p className="empty">No encrypted provider credentials yet.</p>}{replacement ? <form className="form-stack inset" onSubmit={event => { event.preventDefault(); if (replacement.secret) replaceCredential.mutate(replacement); }}><h3>Replace {replacement.provider} credential</h3><label>Credential name<input required value={replacement.name} onChange={event => setReplacement({ ...replacement, name: event.target.value })} /></label><label>New secret / API key<input required type="password" value={replacement.secret} onChange={event => setReplacement({ ...replacement, secret: event.target.value })} /></label><div className="actions"><button className="btn primary" disabled={replaceCredential.isPending || !replacement.secret}>Replace encrypted key</button><button type="button" className="btn" onClick={() => setReplacement(null)}>Cancel</button></div><p className="muted">This is a sensitive action and may require the recent MFA step-up grant from Security.</p></form> : null}</article>
