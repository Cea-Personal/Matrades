...
type Matrix = { id?: string | null; account_id: string; version: number; lanes: MatrixLane[]; schedule: Record<string, unknown>; effective_from?: string | null; restored_from_version?: number | null };
...
export function AccountRules() {
  ...
  const matrixVersions = useQuery<Matrix[]>({ queryKey: ["configuration", "matrix-versions", selectedMatrixAccount], queryFn: () => api(`/accounts/${selectedMatrixAccount}/research-matrix/versions`), enabled: Boolean(selectedMatrixAccount) });
  ...
  const saveMatrix = useMutation({
    ...
    onSuccess: async () => { setMessage("A new research matrix version was activated only for this account."); setMatrixDraft(null); await queryClient.invalidateQueries({ queryKey: ["configuration", "matrix"] }); await queryClient.invalidateQueries({ queryKey: ["configuration", "matrix-versions"] }); await queryClient.invalidateQueries({ queryKey: ["research", "matrix", selectedMatrixAccount] }); },
    onError: (error: Error) => setMessage(error.message),
  });
  const restoreMatrix = useMutation({
    mutationFn: (matrixId: string) => api<Matrix>(`/accounts/${selectedMatrixAccount}/research-matrix/versions/${matrixId}/restore`, { method: "POST" }),
    onSuccess: async result => { setMessage(`Matrix v${result.restored_from_version} restored as v${result.version} for this account only.`); setMatrixDraft(null); await queryClient.invalidateQueries({ queryKey: ["configuration", "matrix"] }); await queryClient.invalidateQueries({ queryKey: ["configuration", "matrix-versions"] }); await queryClient.invalidateQueries({ queryKey: ["research", "matrix", selectedMatrixAccount] }); },
    onError: (error: Error) => setMessage(error.message),
  });
  ...
  return <section className="section-stack"><header><p className="eyebrow">Risk authority</p><h1>Accounts & rules</h1><p className="muted">Current equity and the strictest active prop or internal rule govern every proposal.</p><p className="muted">Removing an account or active risk contributor opens an MFA confirmation dialog. Removals are soft-deleted so audit history remains available.</p></header>
    ...
    <article className="card form-stack"><h2>Account research matrix · {String(activeAccounts.find(item => item.id === selectedMatrixAccount)?.name ?? "No account selected")}</h2><p className="muted">Every account has an independent immutable version sequence. Saving or restoring this matrix cannot change another account’s active matrix.</p>{activeAccounts.length ? <><label>Trading account<select value={selectedMatrixAccount} onChange={event => { setMatrixAccountId(event.target.value); setMatrixDraft(null); }}><option value="">Select account</option>{activeAccounts.map(item => <option key={item.id} value={item.id}>{String(item.name)}</option>)}</select></label><strong>Active version: v{matrix.data?.version ?? 0}</strong>{(matrixDraft ?? matrix.data?.lanes ?? []).map((lane, index) => <label key={`${lane.asset_class}:${lane.instrument_type}`}><input type="checkbox" checked={lane.enabled} onChange={event => setMatrixDraft((matrixDraft ?? matrix.data?.lanes ?? []).map((item, itemIndex) => itemIndex === index ? { ...item, enabled: event.target.checked } : item))} /> {lane.asset_class} · {lane.instrument_type}</label>)}<button className="btn primary" disabled={!selectedMatrixAccount || saveMatrix.isPending || !(matrixDraft ?? matrix.data?.lanes)?.some(item => item.enabled)} onClick={() => saveMatrix.mutate()}>{saveMatrix.isPending ? "Saving…" : `Activate matrix v${(matrix.data?.version ?? 0) + 1} for this account`}</button><h3>Version history for this account</h3>{matrixVersions.isPending ? <p>Loading matrix versions…</p> : matrixVersions.data?.length ? <ul className="record-list">{matrixVersions.data.map(item => <li key={item.id}><strong>Version {item.version}{item.id === matrix.data?.id ? " · Active" : ""}</strong><span>{item.lanes.filter(lane => lane.enabled).length} enabled lanes</span><small>Activated {item.effective_from ? new Date(item.effective_from).toLocaleString() : "—"}{item.restored_from_version ? ` · restored from v${item.restored_from_version}` : ""}</small><button className="btn compact" disabled={!item.id || item.id === matrix.data?.id || restoreMatrix.isPending} onClick={() => item.id && restoreMatrix.mutate(item.id)}>{item.id === matrix.data?.id ? "Active" : `Restore as v${(matrix.data?.version ?? 0) + 1}`}</button></li>)}</ul> : <p className="empty">No saved versions exist for this account. Its first save will create v1.</p>}</> : <p className="empty">Configure a trading account first.</p>}</article>
