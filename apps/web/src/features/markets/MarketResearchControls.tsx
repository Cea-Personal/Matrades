...
type LiteLlmModel = { alias: string; provider_model: string };
...
export function MarketResearchControls({ onReport, onStatus }: { onReport: (report: CoordinatedMarketResearchReport) => void; onStatus: (message: string, error?: boolean) => void }) {
  ...
  const [modelAliases, setModelAliases] = useState<LiteLlmModel[]>([]);
  ...
  const load = useCallback(async () => {
    try {
      const [dashboardResponse, integrationsResponse, scheduleResponse, modelResponse, historyResponse, aliasesResponse] = await Promise.all([
      ...
      if (aliasesResponse.ok) {
        const aliases = await aliasesResponse.json() as { items?: LiteLlmModel[] };
        setModelAliases((aliases.items ?? []).sort((left, right) => left.alias.localeCompare(right.alias)));
      }
